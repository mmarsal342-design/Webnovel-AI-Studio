...
const ChatWindow: React.FC<ChatWindowProps> = ({ storyEncyclopedia }) => {
  const getChatKey = (title: string) => `chatMessages_${title}`;

  const [messages, setMessages] = useState<Message[]>(() => {
    try {
      const saved = localStorage.getItem(getChatKey(storyEncyclopedia.title));
      if (saved) {
  ...
  useEffect(() => {
    chatRef.current = createChatSession(isThinkingMode, storyEncyclopedia);
  }, [isThinkingMode, storyEncyclopedia]);
  
  useEffect(() => {
    localStorage.setItem(getChatKey(storyEncyclopedia.title), JSON.stringify(messages));
  }, [messages, storyEncyclopedia.title]);
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
        ...
        setMessages(prev =>
            prev.map(msg =>
                msg.id === aiMessagePlaceholder.id ? { ...msg, text: "Sorry, I encountered an error. Please try again." } : msg
            )
        );
    } finally {
        setIsLoading(false);
    }
  }, [userInput, isLoading, storyEncyclopedia, isThinkingMode]);
  ...
  return (
    <div className="flex flex-col flex-grow h-full max-h-full bg-slate-800 rounded-lg shadow-2xl overflow-hidden border border-slate-700">
      <div className="p-3 bg-slate-900/50 border-b border-slate-700 flex items-center justify-end gap-4">
         <div className="flex items-center gap-2" title={lang === 'id' ? 'Aktifkan untuk tugas yang lebih kompleks dan kreatif menggunakan model Pro.' : "Enable for more complex and creative tasks using the Pro model."}>
            ...
            <label htmlFor="thinking-mode" className={`text-sm font-medium cursor-pointer transition-colors ${isThinkingMode ? 'text-slate-200' : 'text-slate-400'}`}>
                {lang === 'id' ? 'Mode Berpikir' : 'Thinking Mode'}
            </label>
      ...
      <div ref={chatContainerRef} className="flex-grow p-4 md:p-6 space-y-6 overflow-y-auto">
        {messages.map((msg, index) => (
          <MessageComponent key={msg.id} message={msg} isLoading={isLoading && index === messages.length - 1} />
        ))}
      </div>
      <div className="p-4 bg-slate-800 border-t border-slate-700">
        <div className="relative">
          <textarea
            ...
            placeholder={lang === 'id' ? 'Ayo bangun ceritamu...' : "Let's build your story..."}
